...
class TaskDataProvider {
    ...
    private func logAppsflyerEvent(
        eventName: String,
        eventValues: [String: Any]
    ) {
...
extension TaskDataProvider: TaskDataProviderProtocol {
    ...
    func toggleTaskCompletion(task: Task) -> [Task] {
        ...
        let eventName =
            task.isCompleted
            ? "af_task_marked_completed"
            : "af_task_marked_uncompleted"
        logAppsflyerEvent(
            eventName: eventName,
            eventValues: [
                "task_title": task.title,
                "task_date": task.date.toString(format: "EEE, dd.MM.yyyy"),
            ]
        )
        return taskList
    }
    ...
    func getCurrentDateSelectedTasks(of date: Date) -> [Task] {
        return taskList.filter {
            $0.date.toString(format: "EEE, dd.MM.yyyy")
                == date.toString(format: "EEE, dd.MM.yyyy")
